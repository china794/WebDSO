...
export const GENERATOR = {
    ...
    /** 预设波形类型列表 */
    TYPES: [
        'off', 'sine', 'square', 'triangle', 'sawtooth',
'noise', 'pwm', 'chirp', 'am', 'fm'
    ],
    /** 预设名映射 */
    TYPE_NAMES: {
        off: 'OFF', sine: 'SIN', square: 'SQR', triangle: 'TRI',
        sawtooth: 'SAW', noise: 'NOISE', pwm: 'PWM',
        chirp: 'CHIRP', am: 'AM', fm: 'FM'
    },
    /** 各预置可调参数定义 */
    PARAMS: {
        sine:     ['freq', 'amp', 'phase'],
        square:   ['freq', 'amp', 'duty'],
        triangle: ['freq', 'amp'],
        sawtooth: ['freq', 'amp'],
        noise:    ['amp'],
        pwm:      ['freq', 'amp', 'duty'],
        chirp:    ['freq', 'freqEnd', 'amp', 'sweepTime'],
        am:       ['freq', 'amp', 'modFreq', 'modDepth'],
        fm:       ['freq', 'amp', 'modFreq', 'modDepth']
    },
    /** 各参数默认值 */
    PARAM_DEFAULTS: {
        duty: 50,          // 占空比 0-100%
        phase: 0,          // 相位 0-360°
        freqEnd: 5000,      // chirp 终止频率
        sweepTime: 1,       // chirp 扫描时间 (s)
        modFreq: 10,        // 调制频率
        modDepth: 0.5,      // 调制深度
    },
};
...
export const BYTEBEAT = {
    /** 默认公式（经典交响曲） */
    DEFAULT_CODE: 't*((t>>12|t>>8)&63&t>>4)',
    /** 默认模式 */
    DEFAULT_MODE: 'Bytebeat',
    /** 默认内部采样率 (Hz) */
    ...
    /** 支持的模式 */
    MODES: ['Bytebeat', 'Signed Bytebeat', 'Floatbeat', 'Funcbeat'],
    /** 默认双声道目标通道 */
    DEFAULT_L_CH: 1,
    DEFAULT_R_CH: 2,
    /** worklet 模块路径 */
    WORKLET_URL: './js/bytebeat/audio-processor.js',
    PROCESSOR_NAME: 'webdso-bytebeat'
};
...
export const RENDER = {
    ...
    /** 默认 FFT 增益 - 动态归一化后，增益作相对调节，默认 100 */
    ...
    /** FFT dB 显示下限 (dBFS) - 低于此的噪声底不可见，聚焦信号主体 */
    FFT_DB_FLOOR: -100,
    /** FFT dB 显示上限 (dBFS) - 满幅正弦约 -3dB */
    FFT_DB_CEIL: -3,
    /** FFT 时间平滑系数 (0~1) - 0.5 平滑但响应稍慢，0.2 敏捷 */
    FFT_SMOOTHING: 0.5,
    /** FFT 峰值保持衰减速率 (dB/秒) - 峰值线缓慢下落 */
    FFT_PEAK_DECAY: 20,
    /** FFT 默认是否显示峰值保持线 */
