...
export function refreshInputCard(ch) {
    ...
    // 同步发生器专属参数区显隐
    if (window.__updateGenParamVisibility) window.__updateGenParamVisibility(c.genType || 'off');
...
export function initInputController() {
    ...
    // 视图模式与工具
    function updateTrailControl() {
        const el = document.getElementById('trail-control');
        if (!el) return;
        if (STATE.mode === 'XY') {
            let cnt = 0;
            for (let i = 1; i <= 8; i++) if (STATE['ch' + i]?.on) cnt++;
            el.style.display = cnt >= 3 ? 'flex' : 'none';
        } else {
    ...
    if (DOM.knobTrail) {
        DOM.knobTrail.addEventListener('input', (e) => {
            STATE.view3d.trailLen = parseInt(e.target.value);
            if (DOM.lblTrail) DOM.lblTrail.innerText = STATE.view3d.trailLen + '周期';
        });
    }
    if (DOM.knobCagex) DOM.knobCagex.addEventListener('input', (e) => { STATE.view3d.cageX = parseFloat(e.target.value); if (DOM.lblCagex) DOM.lblCagex.innerText = STATE.view3d.cageX.toFixed(2); });
    if (DOM.knobCagey) DOM.knobCagey.addEventListener('input', (e) => { STATE.view3d.cageY = parseFloat(e.target.value); if (DOM.lblCagey) DOM.lblCagey.innerText = STATE.view3d.cageY.toFixed(2); });
    if (DOM.knobCagez) DOM.knobCagez.addEventListener('input', (e) => { STATE.view3d.cageZ = parseFloat(e.target.value); if (DOM.lblCagez) DOM.lblCagez.innerText = STATE.view3d.cageZ.toFixed(2); });
    ...
    if (DOM.btnRefSave) {
        DOM.btnRefSave.addEventListener('click', function () {
            let activeCh = 1;
            for (let i = 1; i <= 8; i++) {
                if (STATE['ch' + i].on) { activeCh = i; break; }
            }
            STATE.refWave.ch = activeCh;
            STATE.refWave.scale = STATE['ch' + activeCh].scale;
            STATE.refWave.pos = STATE['ch' + activeCh].pos;
            const src = Buffers['pData' + activeCh];
            if (src) {
                STATE.refWave.data = new Float32Array(src);
                STATE.refWave.active = true;
                DOM.btnRefSave.style.display = 'none';
                DOM.btnRefClear.style.display = 'block';
            }
        });
    }
    if (DOM.btnRefClear) {
        DOM.btnRefClear.addEventListener('click', function () {
            STATE.refWave.active = false;
            STATE.refWave.data = null;
            this.style.display = 'none';
            DOM.btnRefSave.style.display = 'block';
        });
    }
    ...
    for (let i = 1; i <= 8; i++) {
        const btn = DOM['ch' + i + 'Toggle'];
        if (btn) btn.addEventListener('click', () => { STATE['ch' + i].on = !STATE['ch' + i].on; refreshVerticalCard(currentSelectedCh); if (window.__updateAutoPan) window.__updateAutoPan(); updateTrailControl(); });
    }
    ...
    if(DOM.btnTrigEn) DOM.btnTrigEn.addEventListener('click', function () {
        ...
        else { this.innerText = '触发: 关'; this.classList.remove('active'); STATE.trigger.frozenIdx = -1; }
    });
    ...
    if (DOM.btnSelUp && DOM.btnSelDn && DOM.numScaleSel) {
        DOM.btnSelUp.addEventListener('click', () => { let v = parseFloat(DOM.numScaleSel.value) || 0; DOM.numScaleSel.value = (v + GENERATOR.MIN_SCALE_STEP).toFixed(2); DOM.numScaleSel.dispatchEvent(new Event('change')); });
        DOM.btnSelDn.addEventListener('click', () => { let v = parseFloat(DOM.numScaleSel.value) || 0; if (v > GENERATOR.MIN_SCALE_STEP) { DOM.numScaleSel.value = (v - GENERATOR.MIN_SCALE_STEP).toFixed(2); DOM.numScaleSel.dispatchEvent(new Event('change')); } });
    }
    ...
    // 高级触发：HF/LF Reject
    if (DOM.btnHfReject) {
        DOM.btnHfReject.addEventListener('click', function () {
            STATE.trigger.hfReject = !STATE.trigger.hfReject;
            this.innerText = STATE.trigger.hfReject ? '开' : '关';
            this.classList.toggle('active', STATE.trigger.hfReject);
        });
    }
    if (DOM.btnLfReject) {
        DOM.btnLfReject.addEventListener('click', function () {
            STATE.trigger.lfReject = !STATE.trigger.lfReject;
            this.innerText = STATE.trigger.lfReject ? '开' : '关';
            this.classList.toggle('active', STATE.trigger.lfReject);
        });
    }
    // 触发释抑 (Holdoff)
    if (DOM.knobHoldoff && DOM.lblHoldoff) {
        bindKnob(DOM.knobHoldoff, DOM.lblHoldoff, null, v => v + '', v => { STATE.trigger.holdoff = v; });
    }
    // 脉宽触发
    if (DOM.trigPulseMode) {
        DOM.trigPulseMode.addEventListener('change', (e) => {
            if (e.target.value === 'off') {
                STATE.trigger.pulseWidth = 0;
            } else {
                STATE.trigger.pulseMode = e.target.value;
                STATE.trigger.pulseWidth = parseInt(DOM.numPulseWidth?.value || 10);
            }
        });
    }
    if (DOM.numPulseWidth) {
        DOM.numPulseWidth.addEventListener('change', (e) => {
            if (STATE.trigger.pulseWidth > 0) {
                STATE.trigger.pulseWidth = parseInt(e.target.value) || 10;
            }
        });
    }
    ...
    if(DOM.btnAutoset) DOM.btnAutoset.addEventListener('click', () => {
        ...
        for (let i = 0; i < CHANNEL_COUNT; i++) { 
            ...
            if (stats[i].valid && ch.on) {
                ...
                targetVPerDiv = Math.max(targetVPerDiv, range / RENDER.CYCLE_DISPLAY_FACTOR);
            }
        }
    ...
    if (DOM.glCanvas) {
        ...
        DOM.glCanvas.addEventListener('touchend', (e) => {
            if (e.touches.length < 2) {
                lastPinchDist = 0;
                lastPanX = 0;
            }
        ...
        DOM.glCanvas.addEventListener('touchstart', (e) => {
            if (e.touches.length === 1) {
                _pressTimer = setTimeout(() => {
                    ...
                    if (DOM.btnRunstop) {
                        if (STATE.run) {
                            DOM.btnRunstop.classList.remove('stopped');
                            DOM.btnRunstop.innerText = '⏸ 暂停';
                        } else {
                            DOM.btnRunstop.classList.add('stopped');
                            DOM.btnRunstop.innerText = '▶ 运行';
                        }
                    }
                    if (DOM.osdRunState) {
                        DOM.osdRunState.innerText = STATE.run ? 'run' : 'stop';
                        DOM.osdRunState.style.background = STATE.run ? 'var(--color-run)' : 'var(--color-stop)';
                    }
                    _pressTimer = null;
                }, 800);
            }
        }, { passive: true });
        DOM.glCanvas.addEventListener('touchend', () => {
            if (_pressTimer) { clearTimeout(_pressTimer); _pressTimer = null; }
        }, { passive: true });
        DOM.glCanvas.addEventListener('touchmove', () => {
            if (_pressTimer) { clearTimeout(_pressTimer); _pressTimer = null; }
        }, { passive: true });

        // 滚轮缩放时基 / Shift+滚轮缩放垂直档位
        DOM.glCanvas.addEventListener('wheel', (e) => {
            e.preventDefault();
            const delta = e.deltaY > 0 ? 1.15 : 1 / 1.15;
            if (e.shiftKey) {
                // Shift+滚轮 → 垂直缩放当前选中通道
                const ch = currentSelectedCh;
                const chState = STATE['ch' + ch];
                if (chState) {
                    const currentVPerDiv = 1 / chState.scale;
                    const newVPerDiv = Math.max(0.01, Math.min(100, currentVPerDiv * delta));
                    chState.scale = 1 / newVPerDiv;
                    if (DOM.numScaleSel) DOM.numScaleSel.value = newVPerDiv.toFixed(2);
                    updateTriggerUI();
                }
                    ...
                    if (xyzCnt >= 3) {
                        ...
                        let currentVal = STATE.secPerDiv || 1.0;
                        currentVal *= delta;
                        currentVal = Math.max(0.01, Math.min(5000, currentVal));
                        STATE.secPerDiv = currentVal;
                        let displayVal = currentVal >= 1000 ? (currentVal / 1000).toFixed(2) + "s" : currentVal.toFixed(2) + "ms";
                        if (DOM.lblTimebase) DOM.lblTimebase.innerText = displayVal;
                    }
    ...
    // 折叠面板：点击 .group-title 切换
    document.querySelectorAll('.hw-group .group-title').forEach(title => {
        title.addEventListener('click', () => {
            const group = title.closest('.hw-group');
            if (!group) return;
