...
export function renderMinimap(theme, viewCtx) {
    ...
    const needUpdate = _minimapCache.lastBufferSize !== bufferSize ||
                       _minimapCache.lastDataVersion !== currentDataVersion;

    if (needUpdate) {
        _minimapCache.lastBufferSize = bufferSize;
        _minimapCache.lastDataVersion = currentDataVersion;
        _minimapCache.data = {};
    }
    ...
    const drawMiniTrace = (data, color, chIdx) => {
        ...
        // 获取或计算降采样数据
        let miniData;
        if (!needUpdate && _minimapCache.data[chIdx]) {
            miniData = _minimapCache.data[chIdx];
        } else {
            ...
            _minimapCache.data[chIdx] = miniData;
        }
